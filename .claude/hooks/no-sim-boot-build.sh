...
if printf '%s' "$cmd" | grep -q 'iOS Simulator' && printf '%s' "$cmd" | grep -q 'name='; then
  emit deny "This plain 'xcodebuild build' targets a concrete simulator (name=…), which cold-boots that device just to compile and clogs the machine. For a compile check run 'make build' (xcodebuild build against generic/platform=iOS Simulator — boots NO simulator). Only use a named destination when you actually need to run tests (make test / make unit-test). See DopaminingSwift/AGENTS.md → Build & test."
fi
