...
This is a monorepo where the root (`/Users/gagzi/Code/DPM`) is **not** a git repo — each platform is its own repo (`DopaminingSwift/`, `cloud-functions/`, `DPM.org/`, `DPMAndroid/`, `admin-board/`, `firebase-admin/`). All git/gh commands MUST run inside the correct sub-repo.
